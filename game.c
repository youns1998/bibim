...
char word[256][256] = { "printf", "scanf", "int", "float", "double", "%d", "%f", "%lf", "return", ";", "&", "main", "#include",//단어 DB
...
void printword() {
	...
	f = rand() % 4 + 1;//색깔을 랜덤으로 생성
	i = rand() % wordmax;
	if (wordi[i] != 1) //단어가 반복해서 나오지 않도록 한다.
	{
		wordi[i] = 1;
		check--;
		gotoxy(x, y);
