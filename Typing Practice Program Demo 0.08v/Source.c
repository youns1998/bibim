...
void _showcursor(int i)
{
	...
	curInfo.bVisible = i; //curInfo는 struct 이므로 bVisible 값을 바꿔준다. (1은 보이고, 0은 안보인다.)
...
int main(int argc,char* argv[])
{
	...
	//파일 열기
	Short_practice_KOR = fopen("Short_practice_KOR.txt", "r");
	Short_practice_ENG = fopen("Short_practice_ENG.txt", "r");
	Long_practice_KOR = fopen("Long_practice_KOR.txt", "r");
	Long_practice_ENG = fopen("Long_practice_ENG.txt", "r");
	Info_practice = fopen("Info_practice.txt", "w+");
